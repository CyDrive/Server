...
func (m *MessageORM) ToPB(ctx context.Context) (Message, error) {
	...
	to.Sender = m.Sender
	to.Receiver = m.Receiver
	to.Type = consts.MessageType(m.Type)
	to.Content = m.Content
	if m.SendedAt != nil {
		to.SendedAt = timestamppb.New(*m.SendedAt)
	}
	to.Expire = m.Expire
...
func DefaultApplyFieldMaskMessage(ctx context.Context, patchee *Message, patcher *Message, updateMask *field_mask.FieldMask, prefix string, db *gorm.DB) (*Message, error) {
	...
	for i, f := range updateMask.Paths {
		...
		if f == prefix+"Sender" {
			patchee.Sender = patcher.Sender
			continue
		}
		if f == prefix+"Receiver" {
			patchee.Receiver = patcher.Receiver
			continue
		}
		if f == prefix+"Type" {
		...
		if !updatedSendedAt && strings.HasPrefix(f, prefix+"SendedAt.") {
			if patcher.SendedAt == nil {
				patchee.SendedAt = nil
				continue
			}
			if patchee.SendedAt == nil {
				patchee.SendedAt = &timestamp.Timestamp{}
			}
			childMask := &field_mask.FieldMask{}
			for j := i; j < len(updateMask.Paths); j++ {
				if trimPath := strings.TrimPrefix(updateMask.Paths[j], prefix+"SendedAt."); trimPath != updateMask.Paths[j] {
					childMask.Paths = append(childMask.Paths, trimPath)
				}
			}
			if err := gorm1.MergeWithMask(patcher.SendedAt, patchee.SendedAt, childMask); err != nil {
				return nil, nil
			}
		}
		if f == prefix+"SendedAt" {
			updatedSendedAt = true
			patchee.SendedAt = patcher.SendedAt
			continue
		}
		if f == prefix+"Expire" {
			patchee.Expire = patcher.Expire
			continue
		}
	}
	if err != nil {
